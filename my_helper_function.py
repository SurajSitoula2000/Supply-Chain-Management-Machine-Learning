...
def rename_data_columns(data, newcol_list):
    ...
    if len(data.columns) != len(newcol_list):
        raise ValueError(f"Number of new columns ({len(newcol_list)}) does not match existing columns ({len(data.columns)}).")
    
    # Show old columns
    print("Old columns:", list(data.columns))
    ...
    # Show new columns
    print("New columns:", list(data.columns))
...
#               '_7_ddate.csv' , '_8_dobject.csv' ]
    ...
    for i in range(len(dnames)):
        try:
            ...
            chunky_dict[chunky_keys[i]] = pd.read_csv(dnames[i])
        except UnicodeDecodeError:
            print("Failed with encoding error, trying again for: ... ", i)
            chunky_dict[chunky_keys[i]] = pd.read_csv(dnames[i], encoding = "ISO-8859-1")
        print("Sucess for: ... ", i)
...
def combine_logistics_and_stability_features(lpi_all, fsi_all):
    ...
    map1 = pd.ExcelFile('C:/Users/lenovo/Desktop/SupplyChainDataset/fsi_lpi_country comps_map.xlsx').parse(sheetname='fsi-lpi-map')
    ...
    lpi_fsi_combined.to_csv('C:/Users/lenovo/Desktop/SupplyChainDataset/lpi_fsi_combined.csv')
...
def generate_factory_location_features(main_data):
    ...
    fact_ref = pd.read_excel('C:/Users/lenovo/Desktop/SupplyChainDataset/factory_map_premadeX.xlsx', encoding = "ISO-8859-1")
